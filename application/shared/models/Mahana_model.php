...
class Mahana_model extends CI_Model
{
    ...
    function send_new_message($sender_id, $recipients, $subject, $body, $priority, $type = 0, $key = null, $client = null)
    {
        ...
        if (count($match_thread)) {
            ...
            $this->reply_to_thread($thread_id, $sender_id, $body, $priority);
     ...
     * @return  integer  $new_msg_id
     */
    function reply_to_thread($thread_id, $sender_id, $body, $priority)
    {
        $this->db->trans_start();
    ...
    function get_thread_messages($user_id, $thread_id, $from_time = false, $status = false, $order_by = 'asc')
    {
        ...
        if ($from_time != false)
        {
            $sql .= ' AND m.cdate > "' . date('Y-m-d H:i:s', $from_time) . '" ';
        }
    ...
    {
        $this->db->select('COUNT(*) AS msg_count');
        $this->db->join('msg_messages', 'msg_messages.id = msg_status.message_id');
        $query = $this->db->where(array('user_id' => $user_id, 'thread_id' => $thread_id, 'status' => $status_id ))->get('msg_status');
