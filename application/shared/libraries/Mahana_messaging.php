...
* send_new_message - will only reply to thread with same participants (sender, receivers)
*
*/
...
class Mahana_messaging
{
     ...
     * @return  array
     */
    function reply_to_message($msg_id, $sender_id, $body = '', $priority = PRIORITY_NORMAL)
    {
        if (empty($sender_id))
        {
            return $this->_invalid_id(MSG_ERR_INVALID_SENDER_ID);
    ...
    /**
     * reply_to_thread() - replies to internal message. This function will NOT create a new thread or participant list
     *
     * @param   integer  $thread_id     REQUIRED
     * @param   integer  $sender_id  REQUIRED
     * @param   string   $body
     ...
     * @return  array
     */
    function reply_to_thread($thread_id, $sender_id, $body = '', $priority = PRIORITY_NORMAL)
    {
        if (empty($sender_id))
        {
            return $this->_invalid_id(MSG_ERR_INVALID_SENDER_ID);
        ...
        if ($new_msg_id = $this->ci->mahana_model->reply_to_thread($thread_id, $sender_id, $body, $priority))
        {
            return $this->_success($new_msg_id, MSG_MESSAGE_SENT);
        }
    ...
    /**
     * get_thread_msg_count() - returns integer with count of thread message for user, by status. defaults to new messages
     *
     * @param   integer  $user_id    REQUIRED
     * @param   integer  $thread_id  REQUIRED
     * @param   integer  $status_id  OPTIONAL - defaults to "Unread"
     * @return  array
     */
    function get_thread_msg_count($user_id, $thread_id, $status_id = MSG_STATUS_UNREAD)
    {
        if (empty($user_id))
        {
            return $this->_invalid_id(MSG_ERR_INVALID_USER_ID);
        ...
        if (is_numeric($message = $this->ci->mahana_model->get_thread_msg_count($user_id, $thread_id, $status_id)))
        {
            return $this->_success($message);
        }
