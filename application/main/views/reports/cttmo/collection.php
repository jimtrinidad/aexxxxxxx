	...
	<div class="row gutter-5">
		...
		<div class="col-xs-12 col-sm-2 offset-top-5">
			<div class="form-group" style="padding: 0 7px">
				<label>Year</label>
				<select class="form-control" onchange="window.location = window.public_url('cttmo/collectionreport?year=' + this.value)">
					<?php
					while ($min < $max) {
		      ...
		      foreach ($reportData['items'] as $k => $item) {
		      	...
		      	echo '<tr>';
		      		...
		      		foreach ($reportData['monthly_total_amount'] as $m => $t) {
		      			...
		      			echo '<td class="text-center">' . ($tt ? '<a href="'.site_url("cttmo/collectiondetails/$sel/$m/{$item['Code']}").'">₱' . number_format($tt) . '</a>' : '') . '</td>';
		      			$item_total += $tt;
		      		}
