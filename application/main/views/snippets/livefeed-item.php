<div class="feedItem hide post-items bg-white padding-10">
   <div class="row gutter-5">
      <div class="col-sm-2 col-xs-3">
         <img src="" class="userAvatar img-responsive" style="margin: 0 auto;">
         <div class="text-center" style="font=family:Trebuchet MS; color:darkred; font-size:12px; padding-top:5px;">
         </div>
      </div>
      <div class="col-sm-10 col-xs-9">
         <div class="row">
            <div class="col-sm-9 col-xs-12">
               <div class="row gutter-5">
                  <div class="col-xs-12 padding-bottom-5">
                     <h2 class="text-bold">
                        <span class="text-blue userFullname">fullname</span>  from
                        <span class="text-green userAddress">address</span>
                        <span class=""> has received a service which is </span>
                        <span class="text-yellow feedServiceName">service name</span> from
                        <span class="text-blue feedDepartmentName">dept name</span>
                     </h2>
                  </div>
                  <div class="col-sm-12 col-xs-8">
                     <div class="transaction-counter">
                        Transaction #:  <span class="feedServiceCode"> some number </span> <br class="visible-xs">
                        Total Services Provided:  <span class="feedServiceProvided"> some number </span> <br>
                        <span class="transaction-date feedServiceDate">date here</span>
                        <div class="transaction-rate visible-xs" style="text-align: left;">
                           <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                           <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                           <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                           <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                           <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                        </div>
                     </div>
                  </div>
                  <div class="col-xs-4 visible-xs">
                     <div class="row gutter-5">
                        <div class="col-md-12 padding-bottom-5">
                           <img src="" class="serviceLogo img-responsive" style="max-width: 70px;float: right;">
                           <div class="clearfix"></div>
                        </div>
                        <div class="col-md-12">
                           <div class="transaction-provider">
                              Service Provided by
                           </div>
                           <div class="transaction-provider-pic">
                              <img src="<?php echo public_url(); ?>resources/images/provider-1.png">
                              <img src="<?php echo public_url(); ?>resources/images/provider-1.png">
                              <img src="<?php echo public_url(); ?>resources/images/provider-1.png">
                           </div>
                        </div>
                     </div>
                  </div>
               </div>
            </div>
            <div class="col-sm-3 hidden-xs">
               <div class="row gutter-5">
                  <div class="col-md-12 padding-bottom-5">
                     <img src="" class="serviceLogo img-responsive" style="max-width: 70px;">
                     <div class="clearfix"></div>
                  </div>
                  <div class="col-md-12">
                     <div class="transaction-rate">
                        <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                        <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                        <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                        <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                        <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                     </div>
                  </div>
                  <div class="col-md-12">
                     <div class="transaction-provider">
                        Service Provided by
                     </div>
                     <div class="transaction-provider-pic">
                        <img src="<?php echo public_url(); ?>resources/images/provider-1.png">
                        <img src="<?php echo public_url(); ?>resources/images/provider-1.png">
                        <img src="<?php echo public_url(); ?>resources/images/provider-1.png">
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
</div>

<!-- <div class="feedItem hide post-items bg-white padding-10">
   <div class="row">
      <div class="col-sm-2 col-xs-3">
         <img src="" class="userAvatar img-responsive">
         <div class="text-center" style="font=family:Trebuchet MS; color:darkred; font-size:12px; padding-top:5px;">
         </div>
      </div>
      <div class="col-sm-10 col-xs-9">
         <div class="row">
            <div class="col-sm-9 col-xs-12">
               <h2 class="text-bold">
               <span class="text-blue userFullname">fullname</span>  from
               <span class="text-green userAddress">address</span>  has
               <span class="text-blue">Applied for a</span>
               <span class="text-yellow feedServiceName">service name</span> from
               <span class="feedDepartmentName">dept name</span>
               </h2>
               <div class="transaction-counter">
                  Transaction #:  <span class="feedServiceCode"> some number </span> <br>
                  Total Services Provided:  <span class="feedServiceProvided"> some number </span>
                  <span class="transaction-date feedServiceDate">date here</span>
               </div>
            </div>
            <div class="col-sm-3 col-xs-12">
               <div class="row gutter-5">
                  <div class="col-md-12 col-sm-12 col-xs-4 padding-bottom-5">
                     <img src="" class="serviceLogo img-responsive" style="max-width: 70px;">
                     <div class="clearfix"></div>
                  </div>
                  <div class="col-md-12 col-sm-12 col-xs-4">
                     <div class="transaction-rate">
                        <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                        <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                        <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                        <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                        <i class="fa fa-star text-yellow" aria-hidden="true"></i>
                     </div>
                  </div>
                  <div class="col-md-12 col-sm-12 col-xs-4">
                     <div class="transaction-provider">
                        Service Provided by
                     </div>
                     <div class="transaction-provider-pic">
                        <img src="<?php echo public_url(); ?>resources/images/provider-1.png">
                        <img src="<?php echo public_url(); ?>resources/images/provider-1.png">
                        <img src="<?php echo public_url(); ?>resources/images/provider-1.png">
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
</div> -->